...
#ifdef STK_GES
#define KEYCODE_RIGHT			KEY_F4
#define KEYCODE_LEFT			KEY_F3
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	{
		als_data = als_data * obj->p_wv_r_bd_ratio / 1024;
	}
	...
	if(obj->p_wv_r_bd_with_co & 0b010)
	{
		if(als_data < 30)
		{
			ir_data = stk3x1x_get_ir_value(obj, 5);
...
static int stk3x1x_enable_als(struct i2c_client *client, int enable)
{
	...
	{		
		err =  stk3x1x_get_ir_value(obj, 2);
		if(err > 0)
			obj->ir_code = err;
	}			
#endif
...
#ifdef CALI_EVERY_TIME
		if( (atomic_read(&obj->ps_high_thd_val) > obj->ps_high_thd_boot) && (obj->boot_cali == 1) )
		{
			atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot); 
			atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot); 
...
static ssize_t stk3x1x_show_ir(struct device_driver *ddri, char *buf)
{
    ...
    reading = stk3x1x_get_ir_value(stk3x1x_obj, 2);
	if(reading < 0)
...
static int stk3x1x_set_input_device(struct stk3x1x_priv *obj)
{
	...
	set_bit(KEYCODE_RIGHT, obj->ges_input_dev->keybit);
	set_bit(KEYCODE_LEFT, obj->ges_input_dev->keybit);
